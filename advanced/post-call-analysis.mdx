...
For each call, Claude reads the transcript and fills in what it can. Missing fields come back as `null` — Claude won't invent data.
...
| `description` | yes | What Claude should look for. Be specific — this drives accuracy. |
...
1. **Base analysis** — always runs. One Claude call returns `call_summary`, `caller_sentiment`, `call_successful`.
2. **Custom extraction** — only runs if the agent has `data_extraction_fields`. A second Claude call returns your custom fields.

Both stages use Claude Haiku and add roughly **$0.001** to the call cost. It's included in the per-minute price — no extra charge.
