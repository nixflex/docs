...
description: "Let the agent call your APIs mid-call"
icon: "wrench"
---
...
| Limit | Value | Why |
|---|---|---|
| Functions per number | 15 | Keeps the tool list focused. Too many functions and the agent gets confused about which to call. |
| URL scheme | HTTPS only | Plain HTTP is rejected for security. |
| Default timeout | 5 seconds | After this, the function is treated as failed. Set the timeout per function, from 1 to 45 seconds, if your API is slow. |
| Response size | 100 KB max | Larger responses are rejected. Big payloads also bloat the agent's context. |
| Tool rounds per turn | Capped | The engine limits how many times functions run in a single turn, so the agent cannot loop on the same function. |
