...
<Note>
  **Telnyx numbers now do almost everything Twilio numbers do** - inbound and outbound
  ...
  Two gaps remain. SMS delivery status is not tracked, so a sent message stays at
  `sent` and never moves to `delivered`. And a Telnyx number cannot be an SMS
  campaign's **From** number - single sends through `POST /v1/sms` work normally, and
  voice campaigns work on both carriers. See
  [Import phone number](/api-reference/phone-numbers/import) for the current support
  table.
</Note>
