...
<Note>
  You don't pass a `from_number`. The engine uses the Twilio number assigned to your agent via the [phone numbers](/api-reference/phone-numbers/import) endpoint. One agent = one phone identity. Import a number and assign it to the agent before making outbound calls.
</Note>
