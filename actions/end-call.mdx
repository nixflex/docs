...
| `speech_unrecognized` | The caller was speaking, but nothing could be understood - usually a language the line is not set to. The engine says so in the agent's language (up to twice), then ends the call. |
