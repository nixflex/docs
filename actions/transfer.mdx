...
| How fast | ~10 seconds | ~10–15 seconds (ring + whisper) |
...
2. The engine dials the destination on the same call - the caller hears normal ringing, 10-second timeout.
3. **When the destination picks up,** they hear a short whisper in your agent's voice — either the static `transfer_whisper` you configured, or an AI-generated 1–2 sentence summary from the conversation. The caller keeps hearing ringing while the whisper plays.
4. After the whisper, the destination is connected directly to the caller.
5. **If the destination doesn't answer:** the caller reconnects to the AI. Same fallback behaviour as cold transfer.
...
## Billing and recording during transfers

Billing stops the moment a transfer connects. The ringing before the answer and the entire transferred conversation are free — you pay only for the AI's part of the call. If a transfer doesn't connect, the call simply continues with the AI and is billed normally throughout. The recording always runs to the very end of the call, so a transferred call's recording is longer than its billed duration — the difference is the free portion.
