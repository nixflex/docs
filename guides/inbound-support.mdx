...
<Steps>
  <Step title="Give the agent your support knowledge">
    ...
    ```text Goes under ## BUSINESS FACTS
    ...
    New patient checkups start from Â£45.
  ...
  <Step title="Text details to the caller mid-call">
    When a caller needs something written down â€” a link, an address, a reference â€” the agent can send it by text while staying on the call. Tell it when to do that in the prompt.
  ...
  <Step title="Transfer to a human when needed">
    ...
    Nixflex supports different handoff styles â€” a straight transfer, or a warm one where the agent briefs the person before connecting. See [Transfer](/actions/transfer) to choose and configure the mode that fits.
  </Step>

  <Step title="Test the whole flow">
    Call your number and try all three paths:

    - Ask a common question â€” the agent should answer it directly.
    - Ask for the address or a link â€” you should receive a text mid-call.
    - Ask to speak to a person â€” the call should transfer.
  </Step>
</Steps>
