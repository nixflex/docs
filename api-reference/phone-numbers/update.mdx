...
---
Updates the `custom_prompt` (business profile) on a number you have already imported. The profile is layered on top of the agent prompt as a **BUSINESS PROFILE** section on inbound calls and SMS replies - see [Phone numbers](/concepts/phone-numbers#per-number-prompt-business-profile).
## Request
```bash
