...
<Note>
  Enabling web calls is free. Calls placed from the browser bill at **$0.09/min**; ordinary phone calls on the same number stay at $0.08/min. Live Monitor follows the same rule - only calls someone listened to bill at $0.09 - and no call ever bills more than $0.09.
</Note>