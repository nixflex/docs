...
| `interruption_sensitivity` | int | No | `50` | `0`-`100`. Lower = harder to interrupt |`n| `boosted_keywords` | string | No | `null` | Comma-separated words your callers say that speech recognition might mishear - staff names, treatments, products, place names. Up to 50. Example: `Dr Patel, hygienist, root canal, Meadow Lane` |
...
<Note>
  ...
  **prompt**, not by a flag. The engine's own rules tell the model to text only if the
  ...
  never mentions texting produces an agent that never texts. `func_send_sms` and
  `func_cold_transfer` are kept because removing a field developers may already send
  would break them silently, but they have no effect.
</Note>
