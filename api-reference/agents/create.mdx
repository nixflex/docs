...
| `response_eagerness` | int | No | `200` | How long the agent waits after the caller stops speaking, in milliseconds. Range `0`-`1000`. Lower is snappier; `0` replies the moment the words are final. Shown as **Response Delay** in the dashboard |
| `boosted_keywords` | string | No | `null` | Comma-separated words your callers say that speech recognition might mishear - staff names, treatments, products, place names. Up to 50. Example: `Dr Patel, hygienist, root canal, Meadow Lane` |
...
| `recording_source` | string | No | `"provider"` | Who records when `record_call` is on. `"provider"` = your carrier records (whole call, carrier recording charges apply). `"nixflex"` = the voice engine records the conversation itself at no extra cost (a transfer to a person is not included). Anything else is rejected with `invalid_recording_source`. See [Call recording](/advanced/call-recording#who-records). |
...
| `require_booking_confirmation` | bool | No | `false` | The engine reads the appointment back and waits for a clear yes before any calendar create, reschedule or cancel. Numbers can override it. See [Booking confirmation](/actions/booking-confirmation). |
