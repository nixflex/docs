...
| `from_number` | string | Yes | Any number you have imported, on **either carrier** - Twilio or Telnyx. The campaign uses that number's own credentials to send, so it must be a number on your account. |
