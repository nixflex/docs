...
<Tip>
  The default voice is **Ashley** and the default language is `multi`. A `multi` agent starts in the language your instructions are written in and will offer to switch if the caller speaks another one. To lock an agent to a single language - including Arabic, Hebrew, Korean, Chinese or Polish - set `language` to that code. See [Languages](/concepts/languages).
</Tip>
...
curl -X POST https://api.nixflex.com/v1/calls/outbound \
  ...
  -d '{
    ...
    "prompt": "You are calling from Acme Dental to check the person can still make their appointment tomorrow at 2pm."
  }'
```
...
<Note>
  `prompt` is required on every outbound call - it is the whole brief for that call, and the agent's own instructions are NOT used on outbound. You don't pass a `from_number`: the engine uses the number you assigned to the agent in Step 3. Outbound works on both carriers. See [Outbound calls](/concepts/outbound-calls).
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Node.js SDK" icon="js" href="/sdks/node">
    The same calls as typed methods with retries and errors handled.
  </Card>
  <Card title="CLI and MCP" icon="terminal" href="/cli/overview">
    Do all of this from a terminal, or from Claude Desktop with the MCP server.
  </Card>
</CardGroup>